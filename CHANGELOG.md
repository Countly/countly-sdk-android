...
* Added `Countly.haltAllInstances()`, which resets every instance, the default one included. It erases the stored data of each instance, so anything recorded but not yet sent is lost.
...
Custom network request headers set through `CountlyConfig.addCustomNetworkRequestHeaders(Map)` are now copied when the SDK initializes. Changing the map you passed in afterwards no longer affects the requests the SDK sends. To change a header while the SDK is running, use `Countly.sharedInstance().requestQueue().addCustomNetworkRequestHeaders(Map)`.
